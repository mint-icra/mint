...
The anonymous review repository does not embed an identity-bearing checkpoint
endpoint. Set `MINT_CHECKPOINT_URL` to the anonymous reviewer artifact URL and
run `scripts/download_assets.sh`; set `HF_TOKEN` as well if that endpoint
requires Hugging Face authentication. The helper downloads only
`model.safetensors`, verifies its checksum, and does not fetch optimizer or
random-state files. You may instead place that file manually at
`checkpoints/model.safetensors` or pass another compatible checkpoint to the
Viewer with `--ckpt`.
...
The asset helper does not download this optional backbone. It downloads only
the MINT checkpoint. Identity-bearing optional robot-description assets are
withheld during double-blind review. MANO remains a separately licensed asset
and is not downloaded by the helper.
