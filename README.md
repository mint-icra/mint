...
This repository is a fresh source snapshot prepared for double-blind review.
It contains no original Git history, author identities, affiliations, personal
contact details, or identity-bearing artifact links. The paper PDF, promotional
film credits, checkpoint/data distribution endpoints, and vendor-identifying
optional robot assets are withheld until the review process is complete. See
[Anonymisation notes](ANONYMISATION.md) for the exact release boundary.

<img src="docs/asset/world_space.webp" width="100%" alt="Eight panels: device ground truth beside MINT prediction in the ego view, in world space, and retargeted to the Dexterous Hand in MuJoCo">
...
| | Item | Where |
| :-- | :-- | :-- |
| ⏳ | MINT checkpoint, 1.139B total parameters | Distribution endpoint withheld during double-blind review |
| ✅ | Inference, Web Viewer, training code | this repository |
| ⏳ | 1,021-hour structured egocentric dataset (non-video portion) | Distribution endpoint withheld during double-blind review |
...
| ⏳ | Optional dexterous-hand assets and retargeting runtime | Vendor-identifying assets withheld during double-blind review |
...
Place the models and assets used by Quick Start at the paths below. MANO must be downloaded from its official website and copied manually.
...
5. Inspect the synchronized GT/Pred 2D view, fixed-world and camera-frame 3D panels, per-frame values, losses, exports, and optional benchmark tools. The export menu can either compose the selected views into one grid MP4 or download 2D, fixed-world 3D, MuJoCo, and Dexterous Hand GT/Pred renders as eight individual MP4 files in a ZIP.
...
The error metrics are transcribed from **Table 2 of the anonymous submission**: HOT3D (27 sequences, 94,978 frames) and the ARCTIC P2 validation split (34 sequences, 25,883 frames). Sequences are evaluated at **full length, with SE(3)-only alignment and no fitted scale** — so scale error is charged rather than absorbed. The arc-length ratio is **GT path length / predicted path length**, averaged equally across sequences: above 1 means the predicted path is too short; below 1 means it is too long. `MegaSaM†` runs without depth refinement. `MINT w/o stage 2` never sees metric ground truth. Bold marks follow the submission; for the arc-length ratio, the target is 1.
...
- **Hand models and simulation** — [MANO](https://mano.is.tue.mpg.de), [SMPL-X](https://smpl-x.is.tue.mpg.de), and [MuJoCo](https://mujoco.org). Vendor-identifying optional retargeting assets are not part of the anonymous review build.
...
MINT's original code is released under the MIT License. Upstream models, datasets, MANO assets, vendored LingBot-Map files, and optional research backends retain their own licenses. Review [Third-party notices](THIRD_PARTY_NOTICES.md) before distribution.
...
During double-blind review, please use the official submission-system channels for questions. Public maintainer details will be restored after review.
