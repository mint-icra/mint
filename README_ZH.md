...
本仓库是为双盲评审重新生成的源码快照，不包含原 Git 历史、作者与单位、个人联络信息或会暴露身份的资产链接。论文 PDF、带署名的宣传片、checkpoint 与数据集发布地址，以及会暴露厂商身份的可选机器人资产会在评审结束后恢复。完整边界见[匿名化说明](ANONYMISATION.md)。
...
| | 内容 | 位置 |
| :-- | :-- | :-- |
| ⏳ | MINT checkpoint，总参数量 1.139B | 双盲评审期间隐藏发布地址 |
| ✅ | 推理、Web Viewer、训练代码 | 本仓库 |
| ⏳ | 1,021 小时结构化第一视角数据集（非视频部分） | 双盲评审期间隐藏发布地址 |
...
| ⏳ | 可选灵巧手资产与重定向运行时 | 双盲评审期间隐藏会暴露厂商身份的资产 |
...
Quick Start 使用的模型与资产请按下表放置。MANO 需由使用者从官方网站下载后手动复制。
...
4. 点击 **开始推理**。
5. 查看同步的 GT/Pred 2D、固定世界和当前相机 3D、逐帧数值、loss、导出及可选 benchmark 工具。导出菜单既可把所选画面合成一支网格 MP4，也可将 2D、固定世界 3D、MuJoCo 和 Dexterous Hand 的 GT/PRED 结果分别导出为 8 支独立 MP4，并打包为 ZIP 下载。
...
下表误差指标摘录自**匿名投稿的 Table 2**：HOT3D（27 条序列、94,978 帧）与 ARCTIC P2 验证集（34 条序列、25,883 帧）。序列按**完整长度评测，只做 SE(3) 对齐、不拟合尺度** —— 因此尺度误差会被计入而不是被吸收掉。弧长比定义为**真值轨迹长度 / 预测轨迹长度（GT / Pred）**，逐序列计算后等权平均：大于 1 表示预测路程偏短，小于 1 表示预测路程偏长。`MegaSaM†` 不带深度精化。`MINT w/o stage 2` 从未见过米制真值。加粗标记沿用投稿；弧长比的目标为 1。
...
由本仓库 Ego 数据生产管线处理得到的 `ego4d`、`egodex` 和 `epickitchen` 非视频数据将在评审结束后发布。仓库已包含数据格式、读取器、准备代码和一份经过审核的小样本，不链接会暴露身份的发布账号。
...
- **手部模型与仿真** — [MANO](https://mano.is.tue.mpg.de)、[SMPL-X](https://smpl-x.is.tue.mpg.de) 与 [MuJoCo](https://mujoco.org)。会暴露厂商身份的可选重定向资产不包含在匿名评审版本中。
...
mint 原创代码使用 MIT License。上游模型、数据集、MANO 资产、内置的 LingBot-Map 源文件及可选研究后端仍遵循各自许可证。发布前请阅读 [第三方声明](THIRD_PARTY_NOTICES.md)。
...
双盲评审期间如有问题，请使用投稿系统提供的正式渠道。公开维护者信息会在评审结束后恢复。
