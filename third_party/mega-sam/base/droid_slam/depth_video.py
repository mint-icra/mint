...
class DepthVideo:
  ...
  def __init__(
    ...
    # Full-resolution disparity is only consumed by the optional upsample path.
    # Avoid reserving several GiB for long videos when upsample is disabled.
    self.disps_up = (
        torch.zeros(buffer, ht, wd, device="cuda", dtype=torch.float).share_memory_()
  ...
  def __item_setter(self, index, item):
    if isinstance(index, int) and index >= self.counter.value:
      ...
      # 一次 .item() 读 max，替代两次（if 判断 + counter 赋值）
      _mx = int(index.max().item())
      if _mx > self.counter.value:
        self.counter.value = _mx + 1
  ...
  def upsample(self, ix, mask):
    ...
    if self.disps_up is None:
      raise RuntimeError("DepthVideo was created with upsample=False")
  ...
  def estimate_preconditor(
    ...
    if t1 is None:
      # 两次 .item() 合成一次 sync
      t1 = int(torch.stack([ii.max(), jj.max()]).max().item()) + 1
