...
class FactorGraph:
  ...
  def __filter_repeated_edges(self, ii, jj):
    ...
    # 批量 D2H：把 2*(E_act + E_inac) + 2*E_new 次 .item() sync 压到 6 次。
    # 原 list-comp 每次 .item() 都等 stream drain，~2ms，E 大时累计到 0.1-0.3s。
    self_ii_cpu     = self.ii.cpu().tolist()
    self_jj_cpu     = self.jj.cpu().tolist()
    self_ii_inac_cpu = self.ii_inac.cpu().tolist()
    self_jj_inac_cpu = self.jj_inac.cpu().tolist()
    ii_cpu = ii.cpu().tolist()
    jj_cpu = jj.cpu().tolist()

    eset = set(zip(self_ii_cpu, self_jj_cpu))
    eset.update(zip(self_ii_inac_cpu, self_jj_inac_cpu))

    keep_list = [(int(i), int(j)) not in eset for i, j in zip(ii_cpu, jj_cpu)]
    keep = torch.tensor(keep_list, dtype=torch.bool, device=ii.device)
  ...
  def update_lowmem(
    ...
    _, rig, ch, ht, wd = self.video.fmaps.shape
    corr_op = AltCorrBlock(self.video.fmaps[:t].view(1, t * rig, ch, ht, wd))
  ...
  def add_proximity_factors(
    ...
    # 原实现每迭代 d[k].item() 一次 sync，循环大小 (t-t0)*(t-t1) 平方级增长。
    # 把 d / ii / jj 一次性搬 CPU，循环内纯 Python 运算；d 的写入同步镜像到
    # CPU 数组上即可（后续迭代读的只是 CPU 拷贝）。
    d_cpu  = d.detach().cpu().numpy().astype(np.float64)
    ii_cpu = ii.detach().cpu().numpy()
    jj_cpu = jj.detach().cpu().numpy()
    ix_cpu = np.argsort(d_cpu)   # 等价于 torch.argsort(d).cpu()，省一次 sync
    for k in ix_cpu:
      if d_cpu[k] > thresh:
        continue
