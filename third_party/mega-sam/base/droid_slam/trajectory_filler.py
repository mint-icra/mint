...
class PoseTrajectoryFiller:
  ...
  def __fill_full(
    ...
    # Encoding every frame as one batch makes the convolution workspace scale
    # with video length and OOMs on otherwise valid long sequences.  Chunking is
    # numerically equivalent in eval mode and writes the same full-video state.
    chunk_size = 32
    self.video.counter.value = len(tstamps)
    for start in range(0, len(tstamps), chunk_size):
      end = min(start + chunk_size, len(tstamps))
      tt = torch.as_tensor(tstamps[start:end], device="cuda")
      image_chunk = torch.stack(images[start:end], 0)
      intrinsic_chunk = torch.stack(intrinsics[start:end], 0)
      depth_chunk = (
          torch.stack(depths[start:end], 0) if depths is not None else None
      )
      mask_chunk = (
          torch.stack(masks[start:end], 0) if masks is not None else None
      )

      inputs = image_chunk[:, :, [2, 1, 0]].to(self.device) / 255.0
      inputs = inputs.sub_(self.MEAN).div_(self.STDV)
      fmap = self.__feature_encoder(inputs)

      self.video[start:end] = (
          tt,
          image_chunk[:, 0],
          Gs.data[start:end],
          1,
