...
class AggregatorBase(nn.Module, ABC):
    ...
    def __init__(
        ...
        logger.debug(f"pretrained_path: {self.pretrained_path}")
    ...
    def _build_patch_embed(
        ...
        if "conv" in patch_embed:
            ...
            # Load pretrained DINOv2 weights only when a path is provided.
            # When empty (the common case where the full backbone is restored
            # later via an outer load_pretrained), skip silently instead of
            # attempting torch.load('') and emitting a misleading warning.
            if pretrained_path:
                try:
                ...
                logger.debug("pretrained_path empty; skipping DINOv2 patch_embed init")
                self._dino_checkpoint = None
