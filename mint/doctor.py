...
def run_checks(profile: str) -> list[Check]:
    ...
    if profile in {"inference", "full"}:
        ...
        hand_body = (
            PROJECT_DIR / "eval" / "simulate" / "dexterous-hand-retargeting" / "dexterous_hand_retargeting"
            / "dexterous-hand-description" / "hand" / "body"
        )
        hand_core = [
            hand_body / kind / f"{side}.{extension}"
            for kind, extension in (("urdf", "urdf"), ("mjcf", "xml"))
            for side in ("left", "right")
        ]
        hand_meshes = list((hand_body / "meshes").glob("*/*.STL"))
        checks.append(Check(
            "optional dexterous-hand retargeting assets",
            all(path.is_file() for path in hand_core) and len(hand_meshes) == 52,
            f"{len(hand_meshes)} STL meshes",
            False,
        ))
