...
def create_app(store) -> Flask:
    ...
    def api_retarget_progress(eid: int):
        ...
        if source not in {"gt", "pred"}:
            abort(400, f"未知 source: {source}")
        if cam_mode not in CAM_MODES:
            abort(400, "Dexterous Hand retargeting 推理参数无效")
    ...
    def retarget_video(eid: int):
        ...
        if source not in {"gt", "pred"}:
            abort(404, f"未知 source: {source}")
        if cam_mode not in CAM_MODES:
            abort(404, "Dexterous Hand retargeting 推理参数无效")
        try:
            ...
            abort(503, f"Dexterous Hand retargeting 依赖不可用: {exc}")
    ...
    def api_export_video(eid: int):
        ...
        filename = (f"mint_item_{eid:03d}_individual_videos.zip"
                    if output_format == "separate" else f"mint_{source_tag}.mp4")
