...
const PANELS = [
  ...
  {id:'dexterous_hand_retarget_3d', name:'Dexterous Hand·Retargeting', kind:'retarget'},
...
const EXPORT_SOURCES = [
  ...
  {id:'dexterous_hand_retarget_3d', label:'Dexterous Hand · Retargeting', file:'dexterous_hand_1'},
];
const FRAME_CAPTURE_SOURCES = [
  ...
  {id:'dexterous_hand_retarget_3d', label:'Dexterous Hand · Retargeting', file:'dexterous_hand_retarget_3d'},
];
const MUJOCO_RENDER_TAG = 'shared_hand_camera_v13_canonical_origin';
const RETARGET_RENDER_TAG = 'shared_hand_camera_v13_no_presence_hud';
...
let state = { eid:0, fps:30, mode:'mesh_skel', nframes:1,
              ...
              hidden:new Set(['mujoco_3d','dexterous_hand_retarget_3d','batch','bench','logdiff']),
                                                        // 仿真/重定向与工具面板均按需打开
              ...
              exportSelection:new Set([
                'both_2d','world_motion_3d','mujoco_3d','dexterous_hand_retarget_3d',
              ]),
...
function buildPanels(preserveTime){
  ...
  for(const id of state.order){
    ...
    if(P.kind==='video'){
      ...
      const methodLabel=isMujoco?'MuJoCo':'Dexterous Hand';
      ...
      const methodDetail=isMujoco?'共用 Dexterous Hand 固定视角':'21点重定向 · 共用 Dexterous Hand 固定视角';
...
// 当前帧取自主视频；MuJoCo 与 Dexterous Hand 只作为整体·2D 的同步跟随画面。
...
function syncMujocoVideos(force=false){
  ...
  for(const id of ['mujoco_3d','dexterous_hand_retarget_3d']){
...
function syncMujocoButton(){
  for(const [selector,id] of [['#mujocoBtn','mujoco_3d'],['#retargetBtn','dexterous_hand_retarget_3d']]){
...
function toggleRetargetPanel(){
  if(state.hidden.has('dexterous_hand_retarget_3d')) state.hidden.delete('dexterous_hand_retarget_3d');
  else state.hidden.add('dexterous_hand_retarget_3d');
  console.log('[btn] Dexterous Hand Retargeting → '+(!state.hidden.has('dexterous_hand_retarget_3d')));
  buildPanels(true);
}
