(() => {
  ...
  const STORAGE_KEY = 'mint-viewer-language';
  ...
  const PHRASES = new Map([
    ...
    ['默认关闭；点击后才将当前 21 点驱动 Dexterous Hand 并启动渲染', 'Off by default. Click to drive Dexterous Hand from the current 21 joints and start rendering.'],
    ...
    ['Dexterous Hand · Retargeting', 'Dexterous Hand · Retargeting'],
    ['有 GT 时两排八格：2D、3D、MuJoCo、Dexterous Hand 均为 GT/PRED', 'With ground truth, export uses eight tiles across two rows: GT/PRED for 2D, 3D, MuJoCo, and Dexterous Hand.'],
    ...
    ['栏目顺序', 'Panel Order'],
    ['拖动 ☰ 调整顺序；点 👁 隐藏/显示对应模块（视频 / 固定世界 / 当前相机 / MuJoCo / Dexterous Hand Retargeting / 数值 / loss / 工具面板）。仿真、retargeting 与工具面板默认关闭；MuJoCo / Dexterous Hand 仅在手动开启后开始渲染', 'Drag ☰ to reorder. Click 👁 to hide or show panels (video / fixed world / current camera / MuJoCo / Dexterous Hand Retargeting / values / loss / tools). Simulation, retargeting, and tool panels are off by default; MuJoCo / Dexterous Hand render only after manual activation.'],
    ['输入目录', 'Input Directory'],
