...
N_VERT = 778   # MANO 顶点数
# MuJoCo and Dexterous Hand retargeting use the same neutral background.  Keep these
...
def robot_ground_half_extent(points: np.ndarray, up=(0.0, 0.0, 1.0)) -> float:
    """Return the shared square ground half-size for MuJoCo and Dexterous Hand scenes."""
    ...
    # MuJoCo first uprights the world while Dexterous Hand keeps the source basis.
...
def fixed_third_camera_pose(up: np.ndarray, hand_points: np.ndarray,
    ...
    """公共固定第三视角：采用 Dexterous Hand 的观察方向，只用完整双手范围决定取景大小。"""
    points = np.asarray(hand_points, dtype=np.float64).reshape(-1, 3)
    points = points[np.all(np.isfinite(points), axis=1)]
...
def canonicalize_robot_coordinates(cameras: np.ndarray,
    ...
    """Use one upright/centered gauge for MuJoCo and Dexterous Hand camera overlays."""
...
<mujoco model="hand_world">
  ...
  <worldbody>
    ...
    def fit_fixed_third_camera(self, up, hand_points, cameras):
        """Fit the shared Dexterous Hand-style third-person camera to the complete hand motion."""
