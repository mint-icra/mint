#!/usr/bin/env python3
"""Render 21-point hands as a fixed third-person first-generation Dexterous Hand video."""
...
_RETARGET_ROOT = _REPO_DIR / "eval" / "simulate" / "dexterous-hand-retargeting"
_DESCRIPTION_ROOT = _RETARGET_ROOT / "dexterous_hand_retargeting" / "dexterous-hand-description"
_CONFIG_DIR = _RETARGET_ROOT / "example" / "config"
...
def _hand_body() -> Path:
    ...
    if all((body / kind / f"{side}.{ext}").is_file()
        ...
        return body
    raise FileNotFoundError(f"Dexterous Hand URDF/MJCF assets not found under: {body}")


def _build_retargeter(side: str):
    if str(_RETARGET_ROOT) not in sys.path:
        sys.path.insert(0, str(_RETARGET_ROOT))
    from dexterous_hand_retargeting import Retargeter

    config_path = _CONFIG_DIR / f"adaptive_analytical_dexterous_hand_{side}.yaml"
    if not config_path.is_file():
        raise FileNotFoundError(f"Dexterous Hand retarget config not found: {config_path}")
...
def _merged_hand_xml(body: Path, width: int, height: int) -> str:
    ...
    return f"""<mujoco model="mint-hand-scene">
...
def _local_to_world_rotation(
    ...
    from dexterous_hand_retargeting.mediapipe import (
        OPERATOR2MANO_LEFT,
        OPERATOR2MANO_RIGHT,
...
def _configure_ego_camera(scene, cam_c2w: np.ndarray,
    ...
    if K.shape != (3, 3) or width <= 0 or height <= 0:
        raise ValueError("Dexterous Hand retargeting camera intrinsics or image size is invalid")
    fx, fy = float(K[0, 0]), float(K[1, 1])
    cx, cy = float(K[0, 2]), float(K[1, 2])
    if fx <= 0 or fy <= 0:
        raise ValueError("Dexterous Hand retargeting camera focal lengths must be positive")
...
class _DexterousHandScene:
    ...
    def __init__(self, body: Path, width: int, height: int):
        ...
        if min(self.floor_body, self.floor_geom, self.light) < 0:
            raise ValueError("Dexterous Hand 场景缺少地面或主光")
        ...
        for side in ("left", "right"):
            ...
            if root < 0:
                raise ValueError(f"Dexterous Hand {side} MJCF root body not found")
            ...
            try:
                ...
                raise ValueError(
                    f"Dexterous Hand {side} URDF/MJCF joint mismatch: {exc.args[0]}"
                ) from exc
            if len(own) != len(pin_names):
                raise ValueError(
                    f"Dexterous Hand {side} DoF mismatch: URDF={len(pin_names)}, "
                    f"MJCF={len(own)}"
                )
    ...
    def solve_pose(self, side: str, joints: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if qpos.shape != config["qpos_perm"].shape or not np.isfinite(qpos).all():
            raise ValueError(
                f"Dexterous Hand {side} retargeting returned invalid qpos {qpos.shape}"
            )
        centered = np.asarray(joints, dtype=np.float64) - joints[0]
    ...
    def render_third(self, validity: dict[str, bool], current_frame: int,
        ...
        if self.third_position is None:
            raise RuntimeError("Dexterous Hand fixed third camera has not been fitted")
        for side, group in _VISUAL_GROUP.items():
            self.option.geomgroup[group] = 1 if validity.get(side) else 0
        self.mujoco.mj_forward(self.model, self.data)
        self.renderer.update_scene(self.data, scene_option=self.option)
...
def _decorate_frame(image: np.ndarray, validity: dict[str, bool],
    ...
    compact = width <= 640
    title = ("HAND RETARGET" if compact else "DEXTEROUS HAND  /  RETARGETING")
    if label_text:
        title += f"  /  {label_text}"
...
def _solve_side_sequence(scene: _DexterousHandScene, side: str, values: np.ndarray,
...
def render_dexterous_hand_video(
    world: dict,
    cam_c2w: np.ndarray,
    kept: np.ndarray,
    output: str | Path,
    *,
    ...
    if (frames <= 0 or cameras.shape != (frames, 4, 4)
        ...
        raise ValueError("Dexterous Hand retargeting camera, joints and validity frame counts differ")
    if K.shape != (3, 3) or len(source_size) != 2 or min(source_size) <= 0:
        raise ValueError("Dexterous Hand retargeting camera intrinsics or source size is invalid")
    ...
    scene = _DexterousHandScene(_hand_body(), render_width, render_height)
    writer = None
    try:
        ...
        if not len(hand_points) or not len(camera_points):
            raise ValueError("Dexterous Hand retargeting 没有可用的手部或相机位置")
            ...
            for frame in range(frames):
                ...
                for side in ("left", "right"):
                    ...
                    if solved_frame != frame:
                        raise RuntimeError(
                            f"Dexterous Hand {side} retargeting 帧顺序错乱: "
...
__all__ = ["render_dexterous_hand_video"]
